...
abbr_to_fips = {v: k for k, v in fips_to_abbr.items()} # Inverse mapping for FIPS lookup
...
for index, row in gdf_hex.iterrows():
    ...
    if state_abbr_from_geojson:
        ...
        print(f"Warning: Missing 'iso3166_2' or identifier for a row: {row.get('name', 'N/A')}")
...
for index, row in df_coordinates.iterrows():
    ax.text(
        row['x'] + 0.01 * (gdf_hex.total_bounds[2] - gdf_hex.total_bounds[0]), # Slight offset for label
        row['y'],
        row['state_abbr'],
        ha='left',
        va='center',
