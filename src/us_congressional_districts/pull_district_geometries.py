...
try:
    ...
    print(
        "This might happen if the ZIP structure is unexpected or the file is corrupted."
    )
    print(
        "Consider downloading the file manually, unzipping it, and then loading the .shp file directly."
    )
    exit()
...
if name_col_shp not in gdf_cd.columns:
    print(
        f"Warning: Expected NAME column '{name_col_shp}' not found. Will proceed without names if it's missing."
    )
...
df_district_coords.rename(columns={"AFFGEOID": "GEO_ID"}, inplace=True)
...
ax.scatter(
    df_district_coords["x"],
    df_district_coords["y"],
    color="red",
    s=5,
    zorder=5,
    label="Centroids",
