...
def read_pl_pop() -> pd.DataFrame:
    ...
    url = (
        "https://www2.census.gov/programs-surveys/decennial/2020/pl/"
        "2020_PLSF_P1.zip"
    )
    zbytes = requests.get(url).content
    with zipfile.ZipFile(io.BytesIO(zbytes)) as z:
...
def build_old_code_list(new_codes: list[str]) -> list[str]:
    ...
    for state, diff in SEAT_CHANGES.items():
        ...
        nums = [
            int(c[-2:])
            for c in new_codes
            if c[len(PREFIX) : len(PREFIX) + 2] == state
        ]
...
# ---------------------------------------------------------------------------
def simulate_117_to_118_crosswalk(
    new_codes: list[str],
    ...
    cross = (
        ...
        .groupby(["code_old", "code_new"], as_index=False)["proportion"]
        .sum()
    )
    ...
    cross = cross.sort_values(
        ["code_old", "proportion"], ascending=[True, False]
    ).reset_index(drop=True)

    # final sanity check
    assert np.allclose(
        cross.groupby("code_old")["proportion"].sum().max(), 1.0
    )
