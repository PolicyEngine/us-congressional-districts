...
def test_sanity():
    ...
    cps = Dataset.from_file(
        Path(get_data_directory() / "input" / "cps" / "cps_2023.h5"), 2023
    )
    ages = pd.read_csv(
        Path(
            get_data_directory()
    ...
    districts_official = pd.read_csv(
        Path(get_data_directory() / "input" / "geographies" / "districts.csv")
    )
    ...
    assert (
        len(
            [
                d
                for d in districts_from_ages
                if d not in list(districts_official.GEO_ID)
            ]
        )
        == 0
    )
    assert (
        len(
            [
                d
                for d in districts_official.GEO_ID
                if d not in list(districts_from_ages)
            ]
        )
        == 0
    )
    ...
    assert np.all(
        (in_age_band_hh >= 0) & (in_age_band_hh == np.floor(in_age_band_hh))
    )
    ...
    persons_in_hh_with_2_in_range_pos = np.where(
        cps_h5["person_household_id"] == hh_with_2_in_range_id
    )
    ...
    assert np.abs(district_estimate - district_target) / district_target < 0.01
    ...
    assert len(
        [d for d in districts_from_ages if d[9:11] == states_from_districts[0]]
    ) == len(districts)

    # Within a district, the weights should not all be clustered on a few households
    hh_single_state_pos = np.where(
        sim.calculate("state_fips").values == int(states_from_districts[0])
    )[0]
    hh_single_state_weights = w[
        np.ix_(district_pos, hh_single_state_pos)
    ].flatten()

    # We'll say (for now) that the weights are sufficiently spread out when the
    # Coefficient of Variation is less than 150%
    assert (
        np.std(hh_single_state_weights) / np.mean(hh_single_state_weights)
        < 1.5
    )
